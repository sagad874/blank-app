...
# --- 4. غرفة العمليات والتواصل السريع (ميزة الاتصال والدردشة البديلة) ---
st.header("📞 غرفة العمليات والتواصل مع فريق BYD العراق")
st.write("إذا واجهتك مشكلة معقدة أو طارئ على الطريق، يمكنك التواصل مباشرة مع الإدارة:")

col_a, col_b = st.columns(2)

with col_a:
    st.markdown("### 💬 مجموعة الفريق")
    st.markdown("[🔗 انضم إلى مجموعة تيليجرام / واتساب لفريق BYD العراق](https://t.me/your_team_link)")

with col_b:
    st.markdown("### ☎️ طوارئ الطريق")
    st.markdown("[📞 اتصال مباشر بطوارئ صيانة الفريق](tel:+9647700000000)")

st.markdown("---")
st.subheader("📝 إرسال بلاغ عطل أو استفسار لإدارة الفريق")
with st.form("support_form"):
    member_name = st.text_input("اسم السائق / العضو:")
    car_model = st.selectbox("موديل السيارة:", ["BYD Song Plus", "BYD Han", "BYD Yuan Plus", "BYD Qin Plus", "أخرى"])
    issue_desc = st.text_area("تفاصيل المشكلة أو الاستفسار:")
    submitted = st.form_submit_button("إرسال البلاغ لقائد الفريق")
    
    if submitted:
        if member_name and issue_desc:
            st.success(f"شكراً لك يا {member_name}! تم إرسال بلاغك بنجاح وسيتواصل معك قائد الفريق قريباً.")
        else:
            st.warning("يرجى ملء الاسم وتفاصيل المشكلة قبل الإرسال.")
