...
st.write("النسخة الميدانية التفاعلية المتقدمة | دليلك التشخيصي وقناة التواصل الرسمية")
...
# --- 3. الدليل الميداني التفاعلي المتفرع للأعطال ---
...
if selected_code and selected_code != "اختر كود من القائمة...":
    ...
    if ans1 == "نعم، وجدْتُ تلفاً/فيوزاً تالفاً":
        ...
        if ans2 == "ضعيفة (11 فولت أو أقل / تحتاج شحن أو تبديل)":
            st.error("🚨 **التشخيص النهائي والحل:** قراءة الـ 11 فولت هي **السبب الجذري للمشكلة!** ضعف البطارية يولد أكواد وهمية كثيرة. قم بشحن البطارية أو استبدالها بأخرى جديدة فوراً، وسيختفي العطل نهائياً بدون إكمال الخطوات.")
        else:
            ...
            ans3 = st.radio("هل اختفى العطل أو تغير بعد تنظيف الفيشة؟", ["نعم، اختفت المشكلة", "لا، ما زال الكود ظاهراً"], key="ans3")
            
            if ans3 == "نعم، اختفت المشكلة":
...
# --- 4. غرفة العمليات والتواصل (واتساب وتيك توك فقط) ---
st.header("📞 التواصل مع فريق BYD العراق")
st.write("انضم إلى مجتمعاتنا الرسمية أو تواصل معنا مباشرة:")
...
with col_a:
    ...
    # ==========================================
    # [!] ضع رابط مجموعة الواتساب هنا بين القوسين
    # ==========================================
    st.markdown("[🔗 انضم إلى مجموعة واتساب لفريق BYD العراق](https://chat.whatsapp.com/ضع_رابط_واتساب_هنا)")

with col_b:
    st.markdown("### 🎬 صفحة تيك توك")
    # ==========================================
    # [!] ضع رابط حساب أو مجموعة تيك توك هنا بين القوسين
    # ==========================================
...
with st.form("support_form"):
    ...
    # تحويل خانة موديل السيارة إلى إدخال نصي حر (Text Input) بدلاً من القائمة المنسدلة
